...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ espera.. se der erro envie o comando novamente.',
					success: '️❬ ✔ ❭ Sucesso 🖤',
					levelon: '❬ ✔ ❭ *leveling* *ativado*',
					leveloff: ' ❬ X ❭  *leveling* *desativado*',
					levelnoton: '❬ X ❭ *leveling não ativado*',
					levelnol: '*error* 0 KKKK',
					error: {
			...
			const ownerNumber = ["5511985598458@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5511985598458@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5511985598458@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5511985598458@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5511985598458@s.whatsapp.net","5511985598458@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
//@nabutoYT					
    ...
    break
            ...
            case 'rapL':
				...
				case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar DEILTON|14`)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO DEILTON BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @DEILTON domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT DEILTON ✨* 」\n*│+ wa.me/556696159143*╰──────*「 *DEILTON* 」*────*\n\n*_SE QUER SER ADMIN DO BOT NABUTO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
		...
		,				fs.unlinkSync(ran)
				...
				case 'setpp3':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isfrends) return reply('Quem é Você?')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Alterado com sucesso o ícone do Grupo')
